import { publicKeyBytesAreOnCurve, publicKeyIsOnCurve } from '../curve';
...
describe('publicKeyIsOnCurve', () => {
    it.each(OFF_CURVE_KEY_BYTES)(
        'returns false when a public key does not lie on the Ed25519 curve [%#]',
        async bytes => {
            expect.assertions(1);
            const publicKey = await crypto.subtle.importKey('raw', bytes, 'Ed25519', true /* extractable */, [
                'verify',
            ]);
            await expect(publicKeyIsOnCurve(publicKey)).resolves.toBe(false);
        }
    );
    it.each(ON_CURVE_KEY_BYTES)('returns true when a public key lies on the Ed25519 curve [%#]', async bytes => {
        expect.assertions(1);
        const publicKey = await crypto.subtle.importKey('raw', bytes, 'Ed25519', true /* extractable */, ['verify']);
        await expect(publicKeyIsOnCurve(publicKey)).resolves.toBe(true);
    });
});

describe('publicKeyBytesAreOnCurve', () => {
    it.each(OFF_CURVE_KEY_BYTES)(
        'returns false when a public key does not lie on the Ed25519 curve [%#]',
        async bytes => {
            expect.assertions(1);
            await expect(publicKeyBytesAreOnCurve(bytes)).resolves.toBe(false);
        }
    );
    it.each(ON_CURVE_KEY_BYTES)('returns true when a public key lies on the Ed25519 curve [%#]', async bytes => {
        expect.assertions(1);
        await expect(publicKeyBytesAreOnCurve(bytes)).resolves.toBe(true);
    });
});
